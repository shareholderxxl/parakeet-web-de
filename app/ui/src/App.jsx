...
// ==== Removed-feature shims (parakeet-web-de: simplified build) ====
const RemoteMicRTC = class { static async connectCached() { return null; } };
function createLiveTranscriber() { return null; }
function VerificationModal() { return null; }
const BoostingTrie = class { static buildFromEncoded() {} };
function compileBoostList() { return null; }
function loadBpeEncoder() { return null; }
function runDiarization() { return null; }
function cancelDiarization() {}
function createDiarizerClient() { return null; }
...
function canonicalizeTurns() { return []; }
function embedSpeakers() { return null; }
function autoNameSpeakers() { return null; }
function restoreBeamWidthAuto() {}
function resolveAutoBeamWidth() { return { greedy: true, width: 1 }; }
function buildBenchmarkReport() { return null; }
function estimatedDownloadMB() { return 0; }
function formatBenchmarkReport() { return ''; }
function planBenchmark() { return null; }
function runBenchmarkPlan() { return null; }
function tilePcm() { return null; }
const BENCHMARK_CLIP = null;
const LONG_PROFILE_TARGET_SEC = 60;
function pickBackendFromProbe() { return null; }
function verdictStillValid() { return false; }
function shouldAutoProbe() { return false; }
function buildVerdict() { return null; }
function planTimedRuns() { return null; }
function median() { return 0; }
const PROBE_MODEL_PATHS = null;
const PROBE_SEQ = 0;
const PROBE_DIM = 0;
const PROBE_INPUT_NAME = '';
const PROBE_WARMUP_RUNS = 0;
const PROBE_INIT_TIMEOUT_MS = 0;
const PROBE_RUN_TIMEOUT_MS = 0;
const DIAR_PALETTE_SIZE = 8;
function requestPersistentStorage() { return Promise.resolve(false); }  // (parakeet-web-de: no-op)
function diarizationModelProtectKeys() { return []; }  // (parakeet-web-de: Diarization entfernt)
...
const dictationEnabled = false; // SpeechMike-Devices deaktiviert (parakeet-web-de)
...
const WASM_ENCODER_QUANTS = ['int4', 'int8'];  // (parakeet-web-de: int4 default, fp32 entfernt)
...
export default function App() {
  ...
  // Encoder precision for the WASM/CPU backend: 'int4' (default; ~391 MB,
  // fast, good quality -- efederici ONNX int4) or 'int8' (SmoothQuant int8,
  // ~840 MB, als Fallback wenn der int4-Encoder fehlt).
  const [wasmEncoderQuant, setWasmEncoderQuant] = useState('int4');
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      try {
        ...
        setWasmEncoderQuant(
          WASM_ENCODER_QUANTS.includes(savedWasmEncoderQuant) ? savedWasmEncoderQuant : 'int4',
        );
  ...
  async function applyBoostSource() {
    console.warn('[parakeet-web-de] applyBoostSource: feature disabled');
  ...
  // (parakeet-web-de: Phrase-Boosting-Worker entfernt)
  ...
  async function loadModel({ useLocalFallback = forceLocalFallback, corruptionRetried = false,
    ...
    try {
      ...
      // (parakeet-web-de: int4-default; fp32-WASM-Pfad entfernt)
      const wasmEncoderRequest = wasmEncoderQuant;
      const downloadOpts = {
      ...
      try {
        ...
        encodePoolInitParamsRef.current = (!wantWebgpu) ? {
          type: 'init',
  ...
  async function stopLiveTranscriberIfRunning() {
    console.warn('[parakeet-web-de] stopLiveTranscriberIfRunning: feature disabled');
    return Promise.resolve(null);
  }
  ...
  async function startRemoteMic() {
    console.warn('[parakeet-web-de] startRemoteMic: feature disabled');
    return Promise.resolve(null);
  }
  ...
  async function processRemoteMicBatch() {
    console.warn('[parakeet-web-de] processRemoteMicBatch: feature disabled');
    return Promise.resolve(null);
  }

  async function stopRemoteMic() {
    console.warn('[parakeet-web-de] stopRemoteMic: feature disabled');
    return Promise.resolve(null);
  }

  async function disconnectRemoteMic() {
    console.warn('[parakeet-web-de] disconnectRemoteMic: feature disabled');
    return Promise.resolve(null);
  }
  ...
  async function initDictationManager() {
    console.warn('[parakeet-web-de] initDictationManager: feature disabled');
    return Promise.resolve(null);
  }
  ...
  // the fix, and says so).
  async function waitForBoostReady() {
    console.warn('[parakeet-web-de] waitForBoostReady: feature disabled');
    return Promise.resolve(null);
  }
  ...
  async function applyBenchmarkCombo() {
    console.warn('[parakeet-web-de] applyBenchmarkCombo: feature disabled');
    return Promise.resolve(null);
  }
  ...
  async function runBenchmark() {
    console.warn('[parakeet-web-de] runBenchmark: feature disabled');
    return Promise.resolve(null);
  }
  ...
  async function diarizeEntry() {
    console.warn('[parakeet-web-de] diarizeEntry: feature disabled');
    return Promise.resolve(null);
  }
  ...
  // (parakeet-web-de: Performance-Probe entfernt - fester WASM-Pfad)
  ...
  async function probeArm() { return null; }  // perf-Probe entfernt
   ...
   */
  async function runPerfProbe() { return null; }  // perf-Probe entfernt
        ...
        <div className="setting-row setting-row--language">
          ...
          <CollapsibleSection id="engine" title={t('settingsGroupEngine')} open={!!sectionsOpen.engine} onToggle={toggleSection}>
            ...
            {(backend === 'wasm' || backend.startsWith('webgpu')) && (() => {
              ...
              // (parakeet-web-de: int4 default, fp32 entfernt)
              const PRECISION_ROW = {
                int4: () => ({ label: t('precisionInt4'), available: !isWebgpu }),
          ...
          {/* (parakeet-web-de: benchmark-Sektion entfernt) */}
